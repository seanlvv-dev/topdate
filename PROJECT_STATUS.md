...
| 05.03 | 导航标签滚动联动 | IntersectionObserver 检测平台数据位置，自动高亮"配对局势"；"仪表盘"和"配对局势"互相切换滚动 |
| 05.03 | 修复匹配算法3个Bug | ①通配符：`any`/`都行`/`不限`在体型和穿搭偏好中视为匹配 ②删除重复的身高检查(代码Bug，永远返回满分) ③卡片拆分为`总分=相似分+地域分`，消除满分假象 |
